    ...
    method! special : special -> ('self_type * special * datatype) =
      let special_inner special = match special with
        ...
        | Handle ({ ih_comp; ih_cases; ih_return; ih_depth }) ->
          ...
          let inner_effects_map = StringMap.fold (fun eff outer_presence_spec map ->
              if StringMap.mem eff inner_effects_map_from_branches then
                map
              else
                StringMap.add eff outer_presence_spec map
