...
let full_kind_of pos prim lin rest =
  ...
  let r = rest in
  SugarKind.mk_unresolved None (Some pk) None (Some l, Some r)
...
let full_subkind_of pos lin res =
  let l = linearity_of_string   pos lin  in
  SugarKind.mk_unresolved None None None (Some l, Some res)
...
let kind_of p =
  ...
  | k          -> SugarKind.mk_unresolved (Some k) None None (None, None)

let named_quantifier name kind freedom = SugarQuantifier.mk_unresolved name (SugarKind.mk_unresolved None None None (None, None)) freedom
...
class_signature:
| SIG OPERATOR COLON datatype SEMICOLON                        { ($2, datatype $4) } (* TODO: consider adding postion args for ind. sigs *)
...
class_declaration:
| CLASS CONSTRUCTOR COLON varlist 
  LBRACE class_signatures RBRACE                               { class_binding ~ppos:$loc($1) (binder ~ppos:$loc($2) $2) $4 $6 }
