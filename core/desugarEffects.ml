      ...
      let o, rv =
        ...
        | D.Open stv
          when (not (SugarTypeVar.is_resolved stv))
            ...
            (* Trying to fix temporal table bug *)
            let _name, (is_eff, kind), freedom = SugarTypeVar.get_unresolved_exn stv in
            let sk = SugarKind.get_unresolved_sk_exn kind in
            let name = SugarKind.get_unresolved_name_exn kind in
            let () = (match name with
            | None -> ()
            | Some n -> Debug.print n) in
